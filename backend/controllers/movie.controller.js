...
export async function getTrendingMovie(req, res) {       // this means fetch that trending movie from the database
    try {
        const data = await fetchFromTMDB("https://api.themoviedb.org/3/trending/movie/day?language=en-US") // this is the url from where we are fetching the trending movies from TMDB
...
export async function getMovieTrailers(req, res) {
    ...
    try {
       const data = await fetchFromTMDB(`https://api.themoviedb.org/3/movie/${id}/videos?language=en-US`)  // fetching the trailers of the movie from the TMDB API and id here is dynamic,
...
export async function getMovieDetails(req, res) {
    const { id } = req.params;  
    try {
        const data = await fetchFromTMDB(`https://api.themoviedb.org/3/movie/${id}?language=en-US`)  // fetching the details of the movie from the TMDB API and id here is dynamic,
...
export async function getSimilarMovies(req, res) {
    const { id } = req.params;
    try {
        const data = await fetchFromTMDB(`https://api.themoviedb.org/3/movie/${id}/similar?language=en-US&page=1`)  // fetching the similar movies from the TMDB API and id here is dynamic,
...
export async function getMoviesByCategory(req, res) {
    ...
    try {
        const data = await fetchFromTMDB(`https://api.themoviedb.org/3/movie/${category}?language=en-US&page=1`)  // fetching the movies by category from the TMDB API and category here is dynamic,
